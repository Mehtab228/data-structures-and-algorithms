...
public class LinkedList<T> {
  ...
  public void insert(T value) {
    Node<T> newHeadNode = new Node<>((String) value);
  ...
  public void append(T value) {
    Node<T> newTailNode = new Node<>((String) value);
    if(head == null) {
  ...
  public void insertBefore(T value, T newValue) {
    ...
    Node<T> current = head;
    Node<T> insertNode = new Node<>((String) newValue);
    while(current.next != null) {
  ...
  public T kthFromEnd(int k) {
    ...
    int position = 0;
    while(current != null) {
      valueMap.put(position++, (T) current.value);
      current = current.next;
    }
    if(k >= valueMap.size())
      throw new IllegalArgumentException("position " + k + " out of bounds");
